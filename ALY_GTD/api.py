...
from .models import REVERT_APPROVES, REVERT_COMMERCIAL_APPROVES, XXALY_FA_ASSET_GTD_V, XXALY_GTD_ACTION_HISTORY, XXALY_GTD_AUDIT_T, XXALY_GTD_DATA_COMPARE_HIST, XXALY_GTD_DATA_COMPARE_T, XXALY_GTD_LOOKUP_DETAIL, XXALY_GTD_LOOKUP_MASTER, XXGDT_PAYABLE_INV_DETS_V, XXGTD_CPI_AUDIT, XXGTD_TRAFFIC_FILE_MASTER, XXGTD_ALLOCATION_INFO, XXGTDRIVERINFO, ApprovalRequest, Attachment, CommercialAttachment, XXGTD_COMMERCIAL_PLATE_INFO, XXGTD_DRIVER_ASSIGNMENT, XXGTD_VEHICLE_INFO, XXGTD_FUEL_INFO, XXGTD_INSURANCE_INFO, XXGTD_PARKING_PERMIT, XXGTD_GPS_TRACKING_INFO, XXGTD_REGISTRATION_INFO, XXGTD_ROAD_TOLL_INFO, XXGTD_COMM_DETAIL_AUDIT, XXGTD_DRIVER_ASSIG_AUDIT, XXGTD_ALLOCATION_AUDIT, XXGTD_FUEL_AUDIT, XXGTD_GPS_TRACKING_AUDIT, XXGTD_INSUR_AUDIT, XXGTD_PARK_PERMIT_AUDIT, XXGTD_REGIS_AUDIT, XXGTD_ROAD_TOLL_AUDIT, XXGTD_VEHICLE_AUDIT, XXGTDAssetAssignment, XXGTDFleetExpenseUpload
...
def create_or_update_fleet_master(
    ...
    FLEET_CREATION_DATE: Optional[str] = Form(None),
    ...
    try:
        with transaction.atomic():
            ...
            if created:
                ...
                if HEADER_ID:
                    fleet_master.HEADER_ID = HEADER_ID
                    fleet_master.FLEET_CONTROL_NO = f"AY-{HEADER_ID}"
                    fleet_master.ACTION = "New"
            else:
                fleet_master = old_fleet_master
                fleet_master.LAST_UPDATED_BY = current_user.username if current_user else "System" 
                if STATUS == "Return for Correction":
                    fleet_master.ACTION = "Return for Correction"
                elif STATUS == "Approved":
                    fleet_master.ACTION = "Approved"
                elif STATUS == "Defleet":
                    fleet_master.ACTION = "Defleet"
                elif STATUS == "Pending for Approval" and fleet_master.STATUS == "Return for Correction":
                    fleet_master.ACTION = "Modified"
                elif STATUS == "Pending for Approval" and fleet_master.ACTION == "New":
                    fleet_master.ACTION = "New"
                elif STATUS == "Cancelled":
                    fleet_master.ACTION = "Sending for Cancellation"  
            ...
            if FLEET_CREATION_DATE:
                FLEET_CREATION_DATE = datetime.strptime(FLEET_CREATION_DATE, '%Y-%m-%d').date()
            else:
                FLEET_CREATION_DATE = date.today()
            ...
            if fleet_master_data["FLEET_CREATION_DATE"] is not None:
                fleet_master.FLEET_CREATION_DATE = fleet_master_data["FLEET_CREATION_DATE"]
                ...
                for file in new_files:
                    ...
                    if base_name not in existing_base_names:
                        file_path = default_storage.save(
                            f'{file_path_prefix}/{new_filename}', 
            ...
            for index, insurance_item in enumerate(insurance_data):
                ...
                for field in insurance_tracked_fields:
                    value = insurance_item.get(field)
                ...
                # Inside the insurance loop where attachments are handled
                ...
                if file_key in request.FILES:
                    ...
                    # Only create attachments for new files
            ...
            for index, allocation_item in enumerate(allocation_data):
                ...
                for field in allocation_tracked_fields:
                    ...
                    if value is not None and value != '':
                        setattr(allocations, field, value)
            ...
            # Handle Driver
            driver_tracked_fields = ['EMPLOYEE_NO', 'EMPLOYEE_NAME', 'DESIGNATION', 'CONTACT_NUMBER', 'ASSIGNMENT_DATE', 'ASSIGNMENT_END_DATE',
            ...
            for index, driver_item in enumerate(driver_data):
                ...
                XXGTD_DRIVER_ASSIG_AUDIT.objects.create(
                    ...
                    ASSIGNMENT_END_DATE=drive.ASSIGNMENT_END_DATE,
            ...
            if STATUS == 'Approved':
                ...
                # Define relevant fields to copy
                RELEVANT_FIELDS = [
                    'HEADER_ID', 'FM_LINE_ID', 'COMPANY_NAME', 'FLEET_CONTROL_NO', 
                    'FLEET_CREATION_DATE', 'VIN_NO', 'MANUFACTURER', 'MODEL',
                    'VEHICLE_TYPE', 'COLOR', 'FLEET_CATEGORY', 'FLEET_SUB_CATEGORY',
                    'ENGINE_NO', 'MODEL_YEAR', 'COUNTRY_OF_ORIGIN', 'SEATING_CAPACITY',
                    'TONNAGE', 'GROSS_WEIGHT_KG', 'EMPTY_WEIGHT_KG', 'PURCHASE_VALUE_AED',
                    'COMMENTS', 'STATUS', 'ApplicationUsage',
                    # Insurance fields
                    'INSURANCE_COMPANY', 'POLICY_NO', 'POLICY_DATE', 'POLICY_EXPIRY_DATE',
                    'PLTS_INS_START_DATE', 'PLTS_INS_EXPIRY_DATE', 'CUR_STAT_MOT_INS',
                    # Registration fields
                    'EMIRATES_TRF_FILE_NO', 'REGISTERED_EMIRATES', 'FEDERAL_TRF_FILE_NO',
                    'REG_COMPANY_NAME', 'TRADE_LICENSE_NO', 'REGISTRATION_NO', 'REGISTRATION_NO1',
                    'REGISTRATION_DATE', 'REG_EXPIRY_DATE', 'CUR_STAT_REG',
                    # Permit fields
                    'PERMIT_TYPE', 'ISSUING_AUTHORITY', 'PERMIT_NO', 'PERMIT_DATE',
                    'PERMIT_EXPIRY_DATE', 'CUR_STAT_PERMIT', 'PermitColor',
                    # GPS fields
                    'GPS_DEVICE_NO', 'GPS_INSTALLATION_DATE', 'GPS_SERVICE_PROVIDER',
                    # Fuel fields
                    'FUEL_TYPE', 'MONTHLY_FUEL_LIMIT', 'FUEL_SERVICE_TYPE', 'FUEL_SERVICE_PROVIDER',
                    'FUEL_DOCUMENT_NO', 'FUEL_DOCUMENT_DATE', 'FUEL_DOC_EXPIRY_DATE', 'CUR_STAT_FUEL_DOC',
                    # Road toll fields
                    'EMIRATES', 'TOLL_TYPE', 'ACCOUNT_NO', 'TAG_NO', 'ACTIVATION_DATE',
                    'ACTIVATION_END_DATE', 'CURRENT_STATUS',
                    # Driver fields
                    'EMPLOYEE_NO', 'EMPLOYEE_NAME', 'DESIGNATION', 'CONTACT_NUMBER',
                    'ASSIGNMENT_DATE', 'TRAFFIC_CODE_NO', 'DRIVING_LICENSE_NO', 'LICENSE_TYPE',
                    'PLACE_OF_ISSUE', 'LICENSE_EXPIRY_DATE', 'GPS_TAG_NO', 'GPS_TAG_ASSIGN_DATE',
                    'ASSIGNMENT_END_DATE',
                    # Allocation fields
                    'DIVISION', 'OPERATING_LOCATION', 'OPERATING_EMIRATES', 'APPICATION_USAGE',
                    'ALLOCATION_DATE', 'ALLOCATION_END_DATE'
                ]
                ...
                for field in XXGTD_VEHICLE_INFO._meta.fields:
                    if field.name in RELEVANT_FIELDS and hasattr(REVERT_APPROVES, field.name):
                        value = getattr(fleet_master, field.name)
                        if value is not None:
                            revert_data[field.name] = value
                ...
                for model, prefix in child_models:
                    instances = model.objects.filter(HEADER_ID=fleet_master.HEADER_ID)
                    for instance in instances:
                        child_revert_data = {
                            'HEADER_ID': fleet_master.HEADER_ID,
                        ...
                        for field in model._meta.fields:
                            if field.name in RELEVANT_FIELDS and hasattr(REVERT_APPROVES, field.name):
                                ...
                                logger.debug(f"Skipping field {field.name} from {model.__name__}")

                        REVERT_APPROVES.objects.create(**child_revert_data)

                # Ensure all records for this HEADER_ID have STATUS 'Approved'
                REVERT_APPROVES.objects.filter(HEADER_ID=fleet_master.HEADER_ID).update(STATUS='Approved')
            ...
            try:
                ...
                for i in range(max_records):
                    ...
                    if driver:
                        ...
                        audit_entry.ASSIGNMENT_END_DATE=driver.ASSIGNMENT_END_DATE
...
def create_or_update_fleet_master(
    ...
    attachment: Optional[Union[UploadedFile, List[UploadedFile]]] = File(None),
    
    PermitAattachment: Optional[Union[UploadedFile, List[UploadedFile]]] = File(None),
    
    GpsAattachment: Optional[Union[UploadedFile, List[UploadedFile]]] = File(None),
    
    FuelDocumentAttachment: Optional[Union[UploadedFile, List[UploadedFile]]] = File(None),
    
    DriverAttachments: Optional[Union[UploadedFile, List[UploadedFile]]] = File(None),
):
    try:
        with transaction.atomic():
            ...
            if not HEADER_ID:
                HEADER_ID = XXGTD_VEHICLE_INFO.get_next_header_id()
                created = True
            else:
                created = False
            if created:
                fleet_master = XXGTD_VEHICLE_INFO(HEADER_ID=HEADER_ID)
            else:
                fleet_master = XXGTD_VEHICLE_INFO.objects.get(HEADER_ID=HEADER_ID)
            ...
            fleet_master_data = {
                key: value for key, value in {
                    ...
                    "MODEL_YEAR": MODEL_YEAR,
                    "COUNTRY_OF_ORIGIN": COUNTRY_OF_ORIGIN,
                    "SEATING_CAPACITY": None if SEATING_CAPACITY in ['null', None, ''] else int(SEATING_CAPACITY),
            ...
            if driver:
                ...
                for index, driver_item in enumerate(driver_data):
                    ...
                    for field in ['EMPLOYEE_NO', 'EMPLOYEE_NAME', 'DESIGNATION', 'CONTACT_NUMBER', 'ASSIGNMENT_DATE','ASSIGNMENT_END_DATE',
                                  'TRAFFIC_CODE_NO', 'DRIVING_LICENSE_NO', 'LICENSE_TYPE', 'PLACE_OF_ISSUE',
                                  'LICENSE_EXPIRY_DATE', 'GPS_TAG_NO', 'GPS_TAG_ASSIGN_DATE']:
...
class AssetInfoSchema(Schema):
    ...
    dep_run_date: Optional[date] = None
    date_placed_in_service: Optional[date] = None

class PayableInfoSchema(Schema):
...
class FleetMasterDetailSchema(FleetMasterSchema):
    ...
    asset_info: Optional[List[AssetInfoSchema]] = None  # Made optional with default None
    payable_info: Optional[List[PayableInfoSchema]] = None  # Made optional with default None
...
def get_fleet_master(request, identifier: str):
    ...
    # Get asset and payable info
    asset_data = list(XXALY_FA_ASSET_GTD_V.objects.filter(
        serial_number=fleet_master.VIN_NO
    ).values(
        'asset_number', 'cost', 'dep_run_date', 'date_placed_in_service'
    ))

     # Create response data first
    response_data = FleetMasterDetailSchema.from_orm(fleet_master)

    # Then add the additional data
    response_data.asset_info = list(XXALY_FA_ASSET_GTD_V.objects.filter(
        serial_number=fleet_master.VIN_NO
    ).values(
        'asset_number', 'cost', 'dep_run_date', 'date_placed_in_service'
    ).distinct())
    ...
    # For payable info
    response_data.payable_info = list(XXGDT_PAYABLE_INV_DETS_V.objects.filter(
        serial_number=fleet_master.VIN_NO
    ).values(
        'INVOICE_DATEDATE', 'COST_TYPEVARCHAR2', 'SUBCATEGORYVARCHAR2', 'REGISTRATIONNOVARCHAR2',
        'INVOICE_NUMVARCHAR2', 'LINE_DESCRIPTIONVARCHAR2'
    ).distinct())
...
# Update the schema to match the model field name
class AssetInfoSchema(Schema):
    asset_number: Optional[str] = None  # Changed from assets_number to asset_number
    cost: Optional[float] = None
    dep_run_date: Optional[date] = None
    date_placed_in_service: Optional[date] = None
...
def get_fleet_info(request):
    ...
    for fleet in fleet_info:
        ...
        asset_data = XXALY_FA_ASSET_GTD_V.objects.filter(
            serial_number=fleet['VIN_NO']
        ).values(
            'asset_number',  # Changed from assets_number to asset_number
            'cost',
            'dep_run_date',
            'date_placed_in_service'
        ).first()
        
        payable_data = XXGDT_PAYABLE_INV_DETS_V.objects.filter(
            serial_number=fleet['VIN_NO']
        ).values(
            'invoice_date',
            'cost_type',
        ...
        fleet_dict['asset_info'] = asset_data if asset_data else None
        fleet_dict['payable_info'] = payable_data if payable_data else None
        
        enhanced_fleet_info.append(fleet_dict)

    return enhanced_fleet_info
...
def get_meaning_options(request, lookup_name: str):
    ...
    return {"meanings": list(sorted_meanings)}   
...
def delete_file(request, model_name: str, instance_id: int, file_index: int):
    try:
        ...
        model_map = {
            'INSURANCE': XXGTD_INSURANCE_INFO,
            'VEHICLE': XXGTD_VEHICLE_INFO,
            'PARKING': XXGTD_PARKING_PERMIT,
            'GPS': XXGTD_GPS_TRACKING_INFO,
            'REGISTRATION': XXGTD_REGISTRATION_INFO,
            'FUEL': XXGTD_FUEL_INFO,
            'ROADTOLL': XXGTD_ROAD_TOLL_INFO,
        ...
        attachment_field_map = {
            'INSURANCE': 'InsurancePolicAattachment',
            'VEHICLE': 'VehiclePurchaseDoc',
            'PARKING': 'PermitAattachment',
            'GPS': 'GpsAattachment',
            'REGISTRATION': 'RegCardAttachment',
            'FUEL': 'FuelDocumentAttachment',
            'ROADTOLL': 'RoadtollAttachments',
        ...
        if model_name == 'VEHICLE':
            instance = XXGTD_VEHICLE_INFO.objects.get(HEADER_ID=instance_id)
            current_files = json.loads(instance.VehiclePurchaseDoc) if instance.VehiclePurchaseDoc else []
            
            if file_index < len(current_files):
                file_path = current_files[file_index]
                file_name = file_path.split('/')[-1]
                current_files.pop(file_index)
                
                # Update VehiclePurchaseDoc
                instance.VehiclePurchaseDoc = json.dumps(current_files)
                instance.save()
                ...
                Attachment.objects.filter(
                    ...
                    Q(file__iexact=file_path)
                ).delete()

        else:
            instance = Model.objects.get(pk=instance_id)
            attachment_field = attachment_field_map.get(model_name)
            attachments = json.loads(getattr(instance, attachment_field) or '[]')
            ...
            if file_index < len(attachments):
                ...
                # Update model's attachment field
                setattr(instance, attachment_field, json.dumps(attachments))
                instance.save()
...
@api.delete("/delete-commercial-file/{model_name}/{instance_id}/{file_index}")
def delete_commercial_file(request, model_name: str, instance_id: int, file_index: int):
    try:
        model_name = model_name.upper()
        ...
        if file_index < len(attachments):
            ...
            # Delete from CommercialAttachment table
            CommercialAttachment.objects.filter(
                Q(file__icontains=file_name) |
                Q(file__iexact=file_path),
                CommercialNumber=instance.COMM_CONTROL_NO
            ).delete()
...
class CommercialMasterSchema(Schema):
    ...
    @validator('LAST_UPDATE_DATE', pre=True)
    def parse_last_update_date(cls, value):
        if value is None:
            return None
...
def create_or_update_commercial_master(
    ...
    COMM_PLATE_DATE: Optional[str] = Form(None),
    ...
    try:
        with transaction.atomic():
            # if COMM_CONTROL_NO:
            #     commercial_master = XXGTD_COMMERCIAL_PLATE_INFO.objects.get(COMM_CONTROL_NO=COMM_CONTROL_NO)
            #     created = False
            # elif HEADER_ID:
            #     commercial_master = XXGTD_COMMERCIAL_PLATE_INFO.objects.filter(HEADER_ID=HEADER_ID).first()
            #     if commercial_master:
            #         created = False
            #     else:
            #         commercial_master = None
            #         created = True
            # else:
            #     new_header_id = XXGTD_COMMERCIAL_PLATE_INFO.get_next_header_id()
            #     commercial_master = XXGTD_COMMERCIAL_PLATE_INFO(HEADER_ID=new_header_id, COMM_CONTROL_NO=f"AY-{new_header_id}")
            #     created = True
            ...
            if COMM_CONTROL_NO:
                old_fleet_master = XXGTD_COMMERCIAL_PLATE_INFO.objects.get(COMM_CONTROL_NO=COMM_CONTROL_NO)
                created = False
            elif HEADER_ID:
                old_fleet_master = XXGTD_COMMERCIAL_PLATE_INFO.objects.filter(HEADER_ID=HEADER_ID).first()
                if old_fleet_master:
                    created = False
                else:
                    old_fleet_master = None
            ...
            if created:
                ...
                commercial_master.CREATED_BY = current_user.username if current_user else "System"
                if HEADER_ID:
                    commercial_master.HEADER_ID = HEADER_ID
                    commercial_master.COMM_CONTROL_NO = f"AY-{HEADER_ID}"
                    commercial_master.ACTION = "New"
            else:
                commercial_master = old_fleet_master
                commercial_master.LAST_UPDATED_BY = current_user.username if current_user else "System"   
                if STATUS == "Return for Correction":
                    commercial_master.ACTION = "Return for Correction"
                elif STATUS == "Approved":
                    commercial_master.ACTION = "Approved"
                elif STATUS == "Defleet":
                    commercial_master.ACTION = "Defleet"
                elif STATUS == "Pending for Approval" and commercial_master.STATUS == "Return for Correction":
                    commercial_master.ACTION = "Modified"
                elif STATUS == "Pending for Approval" and commercial_master.ACTION == "New":
                    commercial_master.ACTION = "New"
                elif STATUS == "Cancelled":
                    commercial_master.ACTION = "Sending for Cancellation"

            if COMM_PLATE_DATE:
                COMM_PLATE_DATE = datetime.strptime(COMM_PLATE_DATE, '%Y-%m-%d').date()
            else:
                COMM_PLATE_DATE = date.today()
            ...
            all_tracked_changes = []
            if not created:
                all_tracked_changes.extend(track_changes(commercial_master, commercial_master_data, tracked_fields, 'XXGTD_COMMERCIAL_PLATE_INFO', current_user, HEADER_ID))
            ...
            # commercial_master.save(new_header_id=new_header_id if created else None)
            ...
            for index, insurance_item in enumerate(insurance_data):
                ...
                for field in insurance_tracked_fields:
                    ...
                    if file_key in request.FILES:
                        ...
                        # Get existing attachment paths
                        existing_attachments = set(CommercialAttachment.objects.filter(
                            commercial_master=commercial_master,
                            attachment_type='InsuranceInfo',
                            HEADER_ID=commercial_master.HEADER_ID
                        ).values_list('file', flat=True))
...
class CommercialMasterSaveSchema(Schema):
    COMPANY_NAME: Optional[str] = None
    COMM_CONTROL_NO: Optional[str] = None
    COMM_PLATE_DATE: Optional[date] = None
    COMM_PLATE_NO: Optional[str] = None
...
class CommercialMasterSaveResponse(Schema):
    message: str
    commercial_master: Optional[CommercialMasterSaveSchema] = None
    
@api.post("/commercial-master/save", response=CommercialMasterSaveResponse)
def create_or_update_commercial_master(
    request,
    COMPANY_NAME:Optional[str] = Form(None),
    HEADER_ID: Optional[str] = Form(None),
    ...
    try:
        with transaction.atomic():
            if not HEADER_ID:
                HEADER_ID = XXGTD_COMMERCIAL_PLATE_INFO.get_next_header_id()
                created = True
            else:
                created = False
            if created:
                commercial_master = XXGTD_COMMERCIAL_PLATE_INFO(HEADER_ID=HEADER_ID)
            else:
                commercial_master = XXGTD_COMMERCIAL_PLATE_INFO.objects.get(HEADER_ID=HEADER_ID)
            ...
            return {
                ...
                "commercial_master": CommercialMasterSaveSchema.from_orm(commercial_master)
            }
...
#     if commercial_master.LAST_UPDATE_DATE:
#         commercial_master.LAST_UPDATE_DATE = commercial_master.LAST_UPDATE_DATE.isoformat()
...
def get_attachments(request, commercial_number: str):
    ...
    return [
        ComAttachmentSchema(
            ...
            upload_date=attachment.upload_date if isinstance(attachment.upload_date, date) else attachment.upload_date.date(),
            uploaded_by=attachment.uploaded_by
        )
        for attachment in attachments
    ]
...
def get_action_history(request, control_number: str):
    try:
        queryset = XXALY_GTD_ACTION_HISTORY.objects.filter(
            ...
            Q(ATTRIBUTE1=control_number) |
            Q(ATTRIBUTE2=control_number)
        ).order_by('-CREATION_DATE')

        action_history = list(queryset.values())

        # Convert date objects to strings and handle empty comments
        for item in action_history:
            for field in ['RESPONSE_DATE', 'CREATION_DATE', 'LAST_UPDATE_DATE']:
                if isinstance(item.get(field), date):
                    item[field] = item[field].isoformat()
            
            # Handle empty comments - convert [] to empty string
            if item.get('RESPONSE_COMMENTS') == '[]' or not item.get('RESPONSE_COMMENTS'):
...
@api.get("/driver-info", response=List[DriverInfoSchema])
def get_driver_info(request):
    driver_info = XXGTDRIVERINFO.objects.values(
        'EMPLOYEE_NO',
        'EMPLOYEE_NAME',
        'DESIGNATION',
        'CONTACT_NUMBER'
    ).order_by('EMPLOYEE_NO').distinct('EMPLOYEE_NO')
    
    return list(driver_info)
...
def send_email(
    ...
    try:
        ...
        current_action = action or data_dict.get('ACTION', 'N/A')
        data_dict['ACTION'] = current_action
...
def revert_data(request, header_id: str):
    try:
        with transaction.atomic():
            ...
            if main_record:
                update_fields = [field.name for field in XXGTD_VEHICLE_INFO._meta.fields 
                               if field.name not in ['FLEET_CONTROL_NO', 'id'] and hasattr(main_record, field.name)]
                for field in update_fields:
                    setattr(fleet_master, field, getattr(main_record, field))
                fleet_master.save(update_fields=update_fields)
            ...
            for model, prefix in [
                ...
                for approved_record in child_records:
                    ...
                    update_data = {field.name: getattr(approved_record, field.name)
                                 for field in model._meta.fields
                                 if hasattr(approved_record, field.name) and field.name not in ['fleet_master', 'id']}
...
@api.get("/assets", response=List[AssetSchema])
def list_assets(request):
    assets = XXGTDAssetAssignment.objects.all()
    return [
        {
            "ASSET_NO": asset.ASSET_NO,
            "ASSET_REG_DATE": asset.ASSET_REG_DATE,
            "LATEST_BOOK_VALUE": asset.LATEST_BOOK_VALUE,
            "BOOK_VALUE_DATE": asset.BOOK_VALUE_DATE
        } for asset in assets
    ]

@api.get("/expenses", response=List[ExpenseSchema])
def list_expenses(request):
    expenses = XXGTDFleetExpenseUpload.objects.all()
    return [
        {
            "EXPENSE_DATE": expense.EXPENSE_DATE,
            "COST_TYPE": expense.COST_TYPE,
            "SUB_CATEGORY": expense.SUB_CATEGORY,
            "VEHICLE_REG_NO": expense.VEHICLE_REG_NO,
