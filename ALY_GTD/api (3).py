...
class CommercialMasterSaveSchema(Schema):
    COMPANY_NAME: str
    COMM_CONTROL_NO: Optional[str] = None
    COMM_PLATE_DATE: Optional[str] = None
...
def send_email(
    ...
    try:
        ...
        if 'STATUS' not in data_dict:
            data_dict['STATUS'] = data_dict.get('fleet_master', {}).get('STATUS', 'N/A')
...
def send_email(
    ...
    try:
        ...
        if 'STATUS' not in data_dict:
            data_dict['STATUS'] = data_dict.get('commercial_master', {}).get('STATUS', 'N/A')
...
def revert_data(request, header_id: str):
    try:
        with transaction.atomic():
            # Get all approved records for this HEADER_ID
            ...
            # Clear REVERT_APPROVES records for this HEADER_ID
...
#             main_record = approved_records.filter(CM_LINE_ID=header_id).first()
#             if main_record:
#                 for field in CommercialMaster._meta.fields:
#                     if hasattr(main_record, field.name):
#                         setattr(commercial_master, field.name, getattr(main_record, field.name))
#                 commercial_master.save()
...
#                     line_id = getattr(approved_record, f'{prefix}_LINE_ID')
#                     try:
#                         instance = model.objects.get(commercial_master=commercial_master, **{f'{prefix}_LINE_ID': line_id})
#                         for field in model._meta.fields:
#                             if hasattr(approved_record, field.name) and field.name != 'commercial_master':
#                                 setattr(instance, field.name, getattr(approved_record, field.name))
#                         instance.save()
#                     except model.DoesNotExist:
#                         # If the record doesn't exist, we don't create a new one
...
#             return JsonResponse({"message": "Data reverted successfully and REVERT_COMMERCIAL_APPROVES cleared"}, status=200)
